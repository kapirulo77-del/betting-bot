...
def load_data() -> dict:
    """Carga los datos desde data.json o devuelve estructura vacía."""
...
def fmt_eur(valor: float) -> str:
    """Formatea un número como euros con signo y separadores españoles."""
    signo = "+" if valor >= 0 else ""
    return f"{signo}{valor:,.2f} €".replace(",", "X").replace(".", ",").replace("X", ".")


def fmt_bank(valor: float) -> str:
    """Formatea el bank sin signo forzado."""
...
def calcular_stats(data: dict) -> dict:
    ...
    # Racha actual (de las más recientes hacia atrás)
    if apuestas:
        ultimo = apuestas[-1]["resultado"]
        racha_tipo = ultimo
        racha_actual = 0
        for ap in reversed(apuestas):
...
def texto_stats(data: dict) -> str:
    ...
    s = calcular_stats(data)

    if s["racha_tipo"] == "ganada":
        racha_str = f"🔥 {s['racha_actual']} victoria{'s' if s['racha_actual'] != 1 else ''} seguida{'s' if s['racha_actual'] != 1 else ''}"
    elif s["racha_tipo"] == "perdida":
        racha_str = f"❄️ {s['racha_actual']} derrota{'s' if s['racha_actual'] != 1 else ''} seguida{'s' if s['racha_actual'] != 1 else ''}"
    else:
        racha_str = "Sin apuestas aún"
...
async def cmd_apuesta(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Calcular beneficio
    if resultado == "ganada":
        beneficio = (cantidad * cuota) - cantidad
    else:
        beneficio = -cantidad
    ...
    msg_apuesta = (
        f"{emoji} Apuesta registrada\n"
        f"Cuota: <b>{cuota}</b> | Cantidad: <b>{fmt_bank(cantidad)}</b>\n"
        f"Resultado: <b>{resultado.upper()}</b>\n"
        f"Beneficio: <b>{signo_ben}{fmt_bank(beneficio)}</b>\n\n"
        + texto_stats(data)
    )
    await update.message.reply_text(msg_apuesta, parse_mode="HTML")
...
async def cmd_historial(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for ap in reversed(ultimas):
        ...
        emoji = "✅" if ap["resultado"] == "ganada" else "❌"
        signo = "+" if ap["beneficio"] >= 0 else ""
        lines.append(
            f"{emoji} <b>{fecha_str}</b>\n"
...
async def cmd_ayuda(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = (
        ...
        "<code>/historial</code> → Últimas 10 apuestas\n\n"
        "⚙️ <b>Otros</b>\n"
...
#    python bot.py
#
# 3. ABRIR TELEGRAM y buscar tu bot por su @username
#
# 4. PRIMER USO:
#    /start         → Inicia el bot
#    /setbank 1000  → Establece tu bank inicial en 1000 €
#    /apuesta 1.75 50 ganada  → Registra tu primera apuesta
#
# 5. DATOS:
#    Los datos se guardan automáticamente en data.json
#    en el mismo directorio donde ejecutas el bot.
#
# 6. DETENER EL BOT:
