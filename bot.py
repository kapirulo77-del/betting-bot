...
def bloque_sistema(titulo: str, emoji: str, s: dict) -> list:
    return [
        SEPARATOR,
        f"{emoji} <b>{titulo}</b>",
        SEPARATOR,
        f"💰 Beneficio neto: <b>{fmt_eur(s['beneficio_neto'])}</b>",
...
def texto_stats(data: dict) -> str:
    ...
    lines = []
    lines += bloque_sistema("SISTEMA RICO", "🔵", s_rico)
    lines += [""]
    lines += bloque_sistema("SISTEMA CAPI", "🟡", s_capi)
    lines += [
        "",
        SEPARATOR,
        "📊 <b>TOTAL COMBINADO</b>",
